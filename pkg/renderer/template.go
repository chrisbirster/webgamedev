...
func NewTemplateRenderer() *TemplateRenderer {
	...
	t.templatesPath = filepath.Join(filepath.Dir(d), "templates")
...
func (t *TemplateRenderer) parse(build *templateBuild) (*TemplateParsed, error) {
	...
	if tp, err = t.Load(build.group, build.key); err != nil {
		...
		parsed := template.New(build.base + ".html").
			Funcs(t.funcMap)

		// Parse all files provided
		if len(build.files) > 0 {
			for k, v := range build.files {
				build.files[k] = fmt.Sprintf("%s/%s%s", t.templatesPath, v, ".html")
			}
		...
		for _, dir := range build.directories {
			dir = fmt.Sprintf("%s/%s/*%s", t.templatesPath, dir, "*.html")
...
func (t *TemplateParsed) Execute(data interface{}) (*bytes.Buffer, error) {
	...
	err := t.Template.ExecuteTemplate(buf, t.build.base+".html", data)
	if err != nil {
		return nil, err
	}
...
func (t *TemplateRenderer) Render(writer io.Writer, name string, data interface{}, c echo.Context) error {
	...
	buf, err := t.Parse().
		...
		Base("main").
		Files(
			// add support for HTMX requests
			fmt.Sprintf("layouts/%s", "main"),
			fmt.Sprintf("pages/%s", name),
		).
		Directories("components").
		Execute(data)

	if err != nil {
		c.Logger().Errorf("Failed to render template: %s, error: %v", name, err)
		return err
	}

	_, err = writer.Write(buf.Bytes())
	if err != nil {
		c.Logger().Errorf("Failed to write rendered template to writer: %v", err)
		return err
	}
